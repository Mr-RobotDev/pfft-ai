...
def generate_text(prompt: str, temperature: float = 0.75) -> str:
    start_time = time.time()  # Start time for measuring response times
    try:
        print(f"Generating text with prompt: {prompt}")
        response = openai.Completion.create(
            model="davinci:ft-ai100-2023-05-20-06-08-29",
            prompt=prompt + " ->",
...
def check_and_retry(prompt: str, model="davinci:ft-ai100-2023-05-20-06-08-29") -> str:
